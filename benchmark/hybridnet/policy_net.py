...
from hetnet import GCNScheduleNet
...
class HybridPolicyNet(nn.Module):
    def __init__(self, selection_mode: str = 'sample', pad_value: bool = True, task_filtering = True):
        """Hybrid Schedule Net
        Args:
    ...
    def _init_gnn(self):
        ...
        out_dim = {'task': 64, 'worker': 64, 'state': 64}     # (hx || cx)
    ...
    def get_variables(self, env):
        ...
        feat_dict = hetgraph_node_helper(curr_g.number_of_nodes(), curr_partialw, curr_partials, durs,
...
class HybridPolicyNetUpdateSelected(nn.Module):
    def __init__(self,
        ...
        self.worker_embedding_size = (self.state_dim + 1) * self.hidden_dim  # worker + state = 64
        self.task_embedding_size = (self.state_dim + 1 + 1) * self.hidden_dim  # unscheduled_task + worker_chosen + state
        self._init_gnn()
        self._init_lstm()
        self._init_classifiers()
    ...
    def forward(self, env: SchedulingEnv):
        ...
        while len(unscheduled_tasks) > 0:
            ...
            # Select Worker
            ## Get Worker Selection Embeddings  len(env.team) = len(worker)
            worker_relevant_embedding = self.merge_embeddings(worker_output, state_output, len(env.team))
            ...
            if self.task_filtering:
                ...
                # np.in1d(array1, array2)用于检查第一个数组arr1中的元素是否存在于第二个数组arr2中，返回一个布尔值的数组;
...
class GPHybridPolicyNet(nn.Module):
    def __init__(self, selection_mode: str = 'sample', pad_value: bool = True,
                 task_filtering=True, verbose='none', device=torch.device("cpu")):
        """Hybrid Schedule Net
        ...
        Args:
            ...
            pad_value (bool, optional): [description]. Defaults to True.
        """
        super(GPHybridPolicyNet, self).__init__()
    ...
    def forward(self, env):
        ...
        # 定义任务数量和工人数量
        num_tasks = env.problem.num_tasks
        num_worker = env.team.num_robots + env.team.num_humans

        # 创建任务数和工人数的搜索空间
        task_space = np.arange(1, num_tasks + 1)  # 任务数量的候选值
        worker_space = np.arange(1, num_worker + 1)  # 工人数量的候选值

        # 创建任务分配问题的搜索空间（所有可能的任务分配组合）
        action_space = [(task, worker) for task in task_space for worker in worker_space]

        g, feat_dict, unscheduled_tasks = self.get_variables(env)

        contexts_output = self.gnn(g, feat_dict)
        context_dim = len(contexts_output)

        length_scale = np.ones(context_dim + num_tasks)
        ...
        optimizer = ContextualBayesianOptimization(all_actions_dict=action_space, contexts=contexts_output, kernel=kernel)
