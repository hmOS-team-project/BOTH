...
if __name__ == '__main__':
    ...
    print(np.mean(np.array(raw_mean)), np.std(np.array(raw_mean)), np.mean(np.array(raw_accu_mean)), np.std(np.array(raw_accu_mean)), np.mean(np.array(feasibility_percentages)), np.std(feasibility_percentages), np.mean(single_instance_times), np.std(single_instance_times))
    
    # 将结果打印到txt文件
    results_file_path = os.path.join(save_folder, "hybridnet_eval_results.txt")
    with open(results_file_path, "w") as file:
        file.write(f"Mean Raw Makespan: {np.mean(np.array(raw_mean))}\n")
        file.write(f"Std Raw Makespan: {np.std(np.array(raw_mean))}\n")
        file.write(f"Mean Accuracy: {np.mean(np.array(raw_accu_mean))}\n")
        file.write(f"Std Accuracy: {np.std(np.array(raw_accu_mean))}\n")
        file.write(f"Mean Inference Time: {np.mean(single_instance_times)}\n")
        file.write(f"Std Inference Time: {np.std(single_instance_times)}\n")
        file.write(f"Feasibility Ratio: {np.mean(np.array(feasibility_percentages))}\n")
        file.write(f"Feasibility Std: {np.std(feasibility_percentages)}\n")
