...
from graph.hetgat import HeteroGCNLayer
...
class HybridScheduleNet(nn.Module):
    ...
    def forward(self, g, feat_dict):
        ...
        Returns:
            [type]: [description]
        """
        # 确保feat_dict中的所有张量都在正确的设备上
        # for key in feat_dict:
        #     feat_dict[key] = feat_dict[key].to(self.device)
        # g = g.to(self.device)
...
# GCN - non-batched version - 3 layer
class GCNScheduleNet(nn.Module):
    def __init__(self, in_dim, hid_dim, out_dim, cetypes):
        super(GCNScheduleNet, self).__init__()

        # Define the input dimension for each node type
        hid_dim_input = {}
        for key in hid_dim:
            hid_dim_input[key] = hid_dim[key]

        # GCN Layers instead of GAT
        self.layer1 = HeteroGCNLayer(in_dim, hid_dim, cetypes)
        self.layer2 = HeteroGCNLayer(hid_dim_input, hid_dim, cetypes)
        self.layer3 = HeteroGCNLayer(hid_dim_input, out_dim, cetypes)
