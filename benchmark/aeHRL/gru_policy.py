...
from graph.lstm_layer import LSTM_CellLayer
...
class GRUPolicyNet(nn.Module):
    def __init__(self, selection_mode: str = 'sample', 
                 pad_value: bool = True, 
                 task_filtering = True,
                 verbose = 'none',
        ...
        """
        super(GRUPolicyNet, self).__init__()
        self.device = device
    ...
    def forward(self, env):
        ...
        # worker_hidden = self.gru_cell_worker.init_hidden(self.num_workers)

        # remove s0 and f0 along with any scheduled
        task_output_ = task_output[unscheduled_tasks + 1]
        # task_hidden = self.gru_cell_task.init_hidden(unscheduled_tasks + 1)
        # 
        ...
        while len(unscheduled_tasks) > 0:
            ...
            chosen_task_embedding = task_output_[index,:].unsqueeze(dim=0)
            task_output = torch.cat((task_output_[:index,:], task_output_[index+1:,:]))
            ...
            # task_output, task_hidden = self.gru_cell_task(chosen_task_embedding, task_hidden)

            # Update the Worker Embedding
            chosen_worker_hidden = worker_output[worker_id].unsqueeze(dim=0)
            worker_out_replacement, chosen_worker_hidden = self.gru_cell_worker(task_worker_embedding, chosen_worker_hidden)
