...
from benchmark.aeHRL.mappo_scheduler import MAPPOScheduler
...
class PPOTrainer(object):
    def __init__(self, scheduler: PPOScheduler, 
                        folder = 'data/small_problem_set', 
        ...
        for i_batch in range(self.start_episode, total_episodes + 1):  # --> (1, 12001)
            ...
            for i_b in range(BATCH_SIZE):
                for step_count in range(num_rounds):
                    ...
                    # Record the reward value obtained in each round of each batch
...
class MAPPOTrainer(object):
    def __init__(self, scheduler: MAPPOScheduler, 
                        folder = 'data/small_problem_set', 
                        resume_training=False, 
                        training_checkpoint=2000,
                        checkpoint_location=None,
...
if __name__ == '__main__':
    ...
    if mode == 'ppo':
        ...
        scheduler = PPOScheduler(device=torch.device(device, device_id), 
                                gamma=GAMMA, lr=lr,weight_decay=weight_decay, lmbda = 0.95, milestones=milestones, lr_gamma=lr_gamma, entropy_coefficient=entropy_coefficient, eps_clip=eps_clip,
                                selection_mode='sample', verbose=verbose)
        
        trainer = PPOTrainer(scheduler, folder=problem_folder, resume_training=resume_training, 
                            training_checkpoint=resume_cp, checkpoint_location=loc)
        
        trainer.train(2, 2, [9, 11], total_episodes, NUM_ROUNDS, from_file=True, start_problem=start_problem,
                      end_problem=end_problem, noise=real_noise, estimator=estimator, est_noise=est_noise)
    
    elif mode == 'mappo':
        # Train Policy Gradient
        scheduler = MAPPOScheduler(device=torch.device(device, device_id), 
                                gamma=GAMMA, lr=lr,weight_decay=weight_decay, lmbda = 0.95, milestones=milestones, lr_gamma=lr_gamma, entropy_coefficient=entropy_coefficient, eps_clip=eps_clip,
                                selection_mode='sample', verbose=verbose)
        
        trainer = MAPPOTrainer(scheduler, folder=problem_folder, resume_training=resume_training, 
                            training_checkpoint=resume_cp, checkpoint_location=loc)
