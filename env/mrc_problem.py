...
class MRCProblem(object):
    ...
    ```
    """
    def __init__(self, num_tasks=20, num_robots=5, num_humans=1, max_diff=3, fname=None,
                 min_dur=15, max_dur=100, max_deadline_multiplier=2.0,
                 prob_deadline=0.15, prob_wait_ori=0.15, noise: bool = False):
        """MRC Problem Initializer2020
        ...
        self.noise = noise
        if fname == None:
            self._generate_problem(num_tasks, num_robots, num_humans, max_diff, min_dur, max_dur,
                                   prob_deadline, prob_wait_ori)
        else:
    ...
    def refactor_problem_from_team(self, team, estimate=False, noise=False):
        ...
        Args:
            ...
            estimate (bool, optional): Boolean to determine if the sampling is done by the estimator or the actual human model.
                                       Defaults to False.
    ...
    def _generate_problem(self, num_tasks: int, num_robots: int, num_humans: int, max_diff: int,
                          min_dur: int, max_dur: int, prob_deadline: float, prob_wait_ori: float):
    ...
    def _initialize_graph(self):
        ...
        for i in range(1, self.num_tasks+1):
            ...
            self.DG.add_weighted_edges_from([(si, 's000', 0), ('f000', fi, 0)])
    ...
    def _generate_durations(self):
        ...
        for i in range(self.num_tasks):
            ...
            # print(gap)
            lower = max(self.min_dur, mean - 3*gap)  # 取两者中的较大值
    ...
    def _generate_ddl(self):
        deadline = round(self.num_tasks * 10 / self.num_robots)  # round()：这是一个取整函数
    ...
    def _generate_diff(self):
        self.diff = np.random.randint(1, self.max_diff + 1, size=self.num_tasks)

    # Generate task locations
    # T x 2:  (x, y)
    def _generate_locs(self):
        self.locs = np.random.randint(1, self.map_width+1, size = (self.num_tasks, 2))
    ...
    def _read_from_file(self, fname, optimizer_read=False):
        ...
        if line_str:
            ...
            self.max_diff = int(split_data[3])
        ...
        self.human_learning_rate = HumanLearningRate(fname=fname + "_human.txt", num_tasks=self.num_tasks,
                                                     num_robots=self.num_robots, num_humans=self.num_humans,
                                                     noise=self.noise)
        self.set_max_makespan()
        ...
        self.diff = np.loadtxt(fname + '_diff.txt', dtype=int)
    ...
    def _read_durations(self, fname):
        for i in range(self.num_tasks):
            ...
            dur_min = self.dur[i].min().item()  # convert from np.int32 to python int
            dur_max = self.dur[i].max().item()
            self.DG.add_weighted_edges_from([(si, fi, dur_max), (fi, si, -1 * dur_min)])
    ...
    def save_to_file(self, fname):
        ...
        print(self.num_tasks, self.num_robots, self.num_humans, self.max_diff, self.min_dur, self.max_dur,
              self.prob_deadline, prob_wait_ori, file=instance_file)
        instance_file.close()
        ...
        np.savetxt(fname + '_diff.txt', self.diff, fmt='%d')
        # np.savetxt(fname+'_loc.txt', self.locs, fmt='%d')

        self.human_learning_rate.save_to_file(fname+'_human.txt')
        
        # Makespan, Passed, Elapsed_time = self.generate_solution(fname+'_gurobi.log')
        ...
        # return True, Makespan, Passed, Elapsed_time
        return True

    def generate_solution(self, filename):
        start = time.time()
        optimal, passed = self.get_optimal_with_gurobi(filename, threads=6)
        # print("Found Optimal Schedule Time:", optimal, passed)
        end = time.time()
        elapsed = end - start

        if passed:
            min_makespan = optimal
        else:
            min_makespan = self.max_deadline
        return min_makespan, passed, elapsed
        # return True
        # try:
        #     # schedule = self.get_optimal_with_gurobi(filename+'_gurobi.log', threads=4)
        #     optimal, passed = self.get_optimal_with_gurobi(filename+'_gurobi.log', threads=4)
