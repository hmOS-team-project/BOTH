...
class SchedulingEnv(gym.Env):
    ...
    def __init__(self, fname: str = None, problem: MRCProblem = None,
                        team: HybridTeam = None, restrict_same_time_scheduling: bool = False,
                        infeasible_coefficient: float = 1.0,
                        noise: bool = False, sample_humans = True):
        """Single Round Scheduling Environment using OpenAI Gym Environment used for simulation /
           and testing of the ScheduleNet Architecture and Variants.
        ...
        self.partials = []   # partial scheduling/separate
        for i in range(len(self.team)):  # self.worker
            self.partials.append(np.zeros(1, dtype=np.int32))

        self.partialw = np.zeros(1, dtype=np.int32)  # partial scheduling/whole

        self.g = self.problem.initialize_STN()
        # get initial min makespan
        self.halfDG = None
        # 检查一致性 --> 获取检查结果，和最短的makespan
    ...
    def _check_consistency_makespan(self, updateDG = True):
        """Check consistency and get min makespan
        ...
        consistent = True
        try:
            p_ultra, d_ultra = johnsonU(self.graph)  # return results_path, actual_dist
        except Exception as e:
            consistent = False
            # print('Infeasible:', e)
                
        '''
        Makespan: Only consider the last finish time of scheduled tasks
        '''
        if consistent:        
            ...
            for k_end in d_ultra[k_start]:
                ...
                if d_ultra[k_start][k_end] < 9999:
                    ...
                    if juDG.has_node(k_start) and juDG.has_node(k_end):
                        juDG.add_edge(k_start, k_end, weight=d_ultra[k_start][k_end])
    ...
    def step(self, ti: int, wi: int, diff=1.0, updateDG=True):
        ...
        self.partialw = np.append(self.partialw, ti)   # 部分调度的任务集合
        ...
        self.graph.add_weighted_edges_from([(si, fi, ti_dur), (fi, si, -1 * ti_dur)])
    ...
    def _calc_reward_discount(self, updateDG = True):
        """Reward R of a state-action pair is defined as the change in objective values after taking the action,
            R = −1 × (Zt+1 − Zt).
