...
from contex_bayes_optimization.kernel_ucb_optimization import KernelUCBOptimizer
...
if __name__ == '__main__':
    ...
    parser.add_argument('--end-no', default=50, type=int)
    ...
    # GP Iteration Count
    parser.add_argument('--num-iterations', default=50, type=int)
    ...
    for count in range(num_repeat):
        ...
        for i_problem in range(1, total_no + 1):
            '''
            ...
            # Record makespan and accuracy metrics of the problem in current iteration
                makespan_accuracy_record = []
                ...
                optimizer = KernelUCBOptimizer(all_actions_dict=discvars, 
                                contexts=contexts, 
                ...
                print('KernelUCB decision starts: Repeat_{}/taskID_{}/BatchID_{}'.format(count, i_problem, i_b))
                for i in tqdm(range(0, num_iterations), position=0):
                    ...
                    pooled_context_output = {k: np.max(np.array(v).reshape(-1, 16), axis=1) for k, v in context_output.items()}

                    context = optimizer.array_to_context(pooled_context_output)
                    action = optimizer.suggest(context)
                ...
                end_t = time.time()
                print('KernelUCB decision-making time: {}'.format(end_t - start_t))
    ...
    # 将结果打印到txt文件
    results_file_path = os.path.join(save_folder, "kernelUCB_eval_results.txt")
